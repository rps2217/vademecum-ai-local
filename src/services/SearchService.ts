...
export class SearchService {
  ...
  async initializeIndex(): Promise<void> {
    try {
      ...
      this.index = allProducts.map(product => {
        ...
        const pathologySearchableText = this.normalizeText(`
          ${formatArrayToString(product.indicaciones, ' ')}
        `);
  ...
  private performTextSearch(normalizedQuery: string, isPathologySearch: boolean): Map<string, { product: Product, score: number }> {
    ...
    const exactRegexes = exactPhrases.map(p => new RegExp(`\\b${p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i'));
    const termsRegexes = searchTerms.map(t => new RegExp(`\\b${t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i'));

    this.index.forEach(item => {
      const textToSearch = isPathologySearch ? item.pathologySearchableText : item.searchableText;
      let matchedExact = exactRegexes.some(r => r.test(textToSearch));

      if (matchedExact) {
        results.set(item.sku, { product: item.product, score: 1.0 });
      } else if (!isPathologySearch && termsRegexes.length > 0) {
        const matchCount = termsRegexes.filter(r => r.test(textToSearch)).length;
        if (matchCount > 0) {
          results.set(item.sku, { product: item.product, score: matchCount / termsRegexes.length });
        }
      }
    });
