...
export const QuotaMonitorService = {
  ...
  start: () => {
    // Escuchar disparadores manuales (ej: por umbral de cambios)
    window.addEventListener('trigger_quota_sync', () => {
      QuotaMonitorService.processQueue().catch(console.error);
    });

    // Revisar cada 10 minutos (600,000 ms)
    setInterval(async () => {
      await QuotaMonitorService.processQueue();
    }, 10 * 60 * 1000);
  }
};
