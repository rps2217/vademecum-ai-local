    ...
    <>
      <div className="fixed bottom-24 sm:bottom-24 left-4 right-4 sm:left-1/2 sm:-translate-x-1/2 bg-card rounded-2xl sm:rounded-full shadow-2xl border border-border p-2 flex flex-col sm:flex-row items-center gap-2 sm:gap-4 z-40 animate-in slide-in-from-bottom-10">
        <div className="flex items-center justify-between w-full sm:w-auto px-2 sm:px-4">
          <div className="flex items-center gap-2">
            <div className="flex -space-x-2">
              ...
              {tray.length > 4 && (
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-card border-2 border-brand-surface flex items-center justify-center text-[9px] sm:text-[10px] font-bold text-muted-foreground">
                  +{tray.length - 4}
                </div>
              )}
            </div>
            <span className="text-xs sm:text-sm font-medium text-foreground ml-1 sm:ml-2">
              {tray.length} <span className="hidden sm:inline">medicamento{tray.length > 1 ? 's' : ''}</span>
            </span>
          </div>
          <button
            onClick={clearTray}
            className="sm:hidden p-2 text-muted-foreground hover:text-muted-foreground"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <button
            onClick={clearTray}
            className="hidden sm:block p-2.5 text-muted-foreground hover:text-muted-foreground hover:bg-background rounded-full transition-colors mr-1"
