      ...
      >
        ...
        <div className="bg-card border border-border p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <Key className={`w-5 h-5 transition-colors ${error ? 'text-red-500' : 'text-muted-foreground'}`} />
                </div>
                <input
                  type="password"
                  value={key}
                  ...
                  placeholder="••••••••"
                  className={`w-full bg-background border ${error ? 'border-red-500/50' : 'border-border'} text-foreground rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-primary/50 transition-all placeholder:text-slate-700 font-mono tracking-widest`}
                  autoFocus
                />
              </div>
              {error && (
            ...
            <button
              type="submit"
              className="w-full bg-primary hover:bg-primary text-foreground font-bold py-4 rounded-2xl transition-all shadow-lg shadow-brand-primary/20 flex items-center justify-center gap-2 group"
            >
