  ...
  return (
    <section className="space-y-4">
      <h3 className="text-xs font-bold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2">
        <Copy className="w-4 h-4" /> Alternativas Terapéuticas o Bioequivalentes
      </h3>
      <div className="grid grid-cols-1 gap-3">
        {products.map(altProduct => (
          <button
            key={altProduct.sku}
            onClick={() => onProductClick?.(altProduct)}
            className="flex items-center gap-4 p-4 bg-emerald-500/5 hover:bg-emerald-500/10 rounded-2xl border border-emerald-500/10 transition-all group text-left"
          >
