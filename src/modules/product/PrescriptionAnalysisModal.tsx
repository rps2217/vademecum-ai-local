...
export const PrescriptionAnalysisModal: React.FC<PrescriptionAnalysisModalProps> = ({ products, onClose }) => {
  ...
  const getRiskColor = (risk: string) => {
    switch (risk) {
      ...
      default: return 'text-muted-foreground bg-background0/10 border-slate-500/20';
    }
  };
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      default: return 'bg-background0/10 text-muted-foreground border-slate-500/20';
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end p-2 md:p-4 bg-black/60 animate-in fade-in duration-300" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-card w-full h-full rounded-[2rem] shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col md:flex-row overflow-hidden border border-border">
        ...
        <div className="w-full md:w-1/3 p-6 md:p-10 overflow-y-auto border-r border-border bg-background scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
          <div className="flex justify-between items-start mb-10">
            ...
            <button 
              onClick={onClose}
              className="p-3 rounded-xl bg-card hover:bg-slate-700 text-muted-foreground transition-all border border-border md:hidden"
            >
              <X className="w-8 h-8" />
            </button>
          </div>
          ...
          <div className="space-y-4">
            {products.map(product => (
              <div key={product.sku} className="bg-card p-5 rounded-3xl border border-border shadow-sm hover:shadow-lg transition-all border-l-4 border-l-brand-primary/40 group">
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-primary rounded-2xl text-primary mt-1 group-hover:scale-110 transition-transform">
                    <Pill className="w-6 h-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-lg font-extrabold text-foreground truncate leading-tight">{product.nombre_comercial}</h4>
                    <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest mt-0.5">{formatArrayToString(product.principios_activos, ', ')}</p>
        ...
        <div className="w-full md:w-2/3 bg-background p-6 md:p-12 flex flex-col relative overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
          <button 
            onClick={onClose}
            className="absolute top-10 right-10 p-3 rounded-xl bg-card hover:bg-slate-700 text-muted-foreground transition-all border border-border hidden md:flex items-center justify-center z-10"
          ...
          <div className="mb-10 pr-16 relative">
            <div className="absolute -top-20 -left-20 w-64 h-64 bg-primary blur-[100px] rounded-full pointer-events-none" />
            <h3 className="text-3xl font-extrabold text-foreground mb-2 flex items-center gap-3 relative z-10">
              <Sparkles className="w-8 h-8 text-primary" />
              Reporte de Seguridad IA
            </h3>
            <p className="text-lg text-muted-foreground font-medium relative z-10">Análisis exhaustivo de riesgos e interacciones bioquímicas.</p>
          </div>
          
          <div className="flex-1 space-y-10 relative z-10">
            {isAnalyzing ? (
              <div className="flex flex-col items-center justify-center py-[15vh] text-muted-foreground">
                <Loader2 className="w-16 h-16 animate-spin mb-8 text-primary opacity-50" />
                <p className="text-2xl font-extrabold text-foreground animate-pulse tracking-tight">Cruciando datos bioquímicos...</p>
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                ...
                <div className={`p-10 rounded-[2.5rem] border-2 flex flex-col md:flex-row gap-10 items-center overflow-hidden relative ${getRiskColor(analysis.riesgo_total)}`}>
                  ...
                  <div className="flex-1 text-foreground font-serif italic text-2xl leading-tight border-l border-border/50 md:pl-10">
                ...
                {analysis.interacciones.length > 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      {analysis.interacciones.map((inter, idx) => (
                        <div key={idx} className="glass-panel rounded-[2rem] p-8 space-y-6 relative overflow-hidden group">
                          ...
                          <div className="flex items-center justify-between relative z-10">
                            <div className="flex flex-wrap gap-2">
                              {inter.productos.map(p => (
                                <span key={p} className="px-3 py-1 bg-background border border-border rounded-xl text-[10px] font-black uppercase tracking-widest text-primary/80">
                                  {p}
                                </span>
                              ))}
                            </div>
                          ...
                          <p className="text-xl font-bold text-foreground leading-snug relative z-10">{inter.descripcion}</p>
                          <div className="bg-background p-6 rounded-2xl border border-border flex items-start gap-4 relative z-10">
                  ...
                  <div className="glass-panel p-16 rounded-[3rem] text-center space-y-6 relative overflow-hidden">
                    ...
                    <div className="w-20 h-20 bg-brand-accent text-primary rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-brand-accent/20 relative z-10 rotate-3 animate-pulse">
                ...
                <section className="pt-16 border-t border-border">
                  <div className="mb-10">
                    <h3 className="text-2xl font-extrabold text-foreground mb-2">Consulta Especializada</h3>
                    <p className="text-muted-foreground font-medium italic">Inicia un diálogo con el Asistente Clínico para desglosar este reporte.</p>
                  </div>
